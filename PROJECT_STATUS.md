...
7. **Local languages** — IN PROGRESS. `LanguageContext` now supports English, Spanish, French, Yoruba, Igbo, Hausa **and Nigerian Pidgin (new)**; `t()` falls back to English for missing keys. Dictionary expanded with nav + common-action keys; the always-visible **BottomNav is now translated** (switching language in Personalization updates it app-wide). Remaining: apply `t()` across the rest of the pages incrementally (page headers, forms), and translate more strings. **Streaks/adherence** still to do.
