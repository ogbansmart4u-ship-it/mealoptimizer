...
### B. Mobile / UI polish
- **Home Water Tracker fix (was static).** The widget was `useState(5)` — reset on every reload, never saved, not date-aware. Now persisted per calendar day in `localStorage` (`water-glasses-YYYY-MM-DD`): starts at 0 each new day, keeps the count across reloads, add/remove glasses toward the 8-glass goal, and rolls over to the new day on tab refocus. (The dedicated `/hydration` page remains the full backend-backed tracker.) **Market Update** banner now rotates through 12 location-aware produce/market tips by day-of-year so it refreshes daily instead of showing one fixed line.
- **Design system foundation (new).** Added `DESIGN_GUIDELINES.md` (adapted from the uploaded UI/UX Master Blueprint — 8pt grid, type scale, colour, component specs, motion, a11y, QA checklist). Added missing tokens to `src/styles/theme.css`: `--secondary-accent` (amber `#F59E0B`), `--success`, `--warning`, `--danger`, `--ink` (`#1E293B`), exposed as Tailwind utilities (`bg-secondary-accent`, `text-warning`, etc.). **Teal unified (done):** all `#0F766E`/`#115E59` migrated to `#1f7a8c`/`#1a6273` across auth/landing pages, and the `--primary`/`--ring` tokens set to `#1f7a8c` — so every token-based component (shadcn Button default, focus rings) is now the one brand teal. **Core components verified against spec:** Button (44/48px heights, focus ring, token colours), Input (44px, rounded, 16px padding, focus ring), and BottomNav (line→filled active icon + heavier active stroke, ≥44px touch targets, 12px labels) already meet the blueprint. Remaining UI polish: per-screen spacing/typography passes, and applying `text-warning`/`bg-secondary-accent` where badges/alerts belong.
- **Loading skeletons (done across data screens).** Swapped plain spinners/"Loading…" text for `SkeletonLoader` components on: Biometric Dashboard (`SkeletonDashboard`), Glucose Insights, Grocery List, Hydration, Sleep, Symptoms (`SkeletonList`), and Weight history (`SkeletonRows`, a new compact in-card row skeleton). Goals already used skeletons. Any remaining data page follows the same pattern: import from `../components/SkeletonLoader`, render in the `loading` branch. Verified via esbuild parse (sandbox couldn't run a full `vite build` this session).
- **Motion & micro-interactions (blueprint §6, done).** Subtle cross-fade on every route change (`routes.tsx` RootLayout, opacity-only so it never breaks `position:fixed` nav/FABs; respects reduced-motion). Light tap haptics via `navigator.vibrate(8)` on buttons/links (`main.tsx`, Android/Chrome only, feature-detected, skips disabled + reduced-motion). **Safe-area fix:** bottom nav now pads `env(safe-area-inset-bottom)` so it clears the phone home-bar. Out of scope for web PWA: native widgets, Metal/WebGL shaders, iOS haptics.
...
- ~~Reduce the frontend bundle (~1.7 MB) via code-splitting~~ **DONE.** All routes are `React.lazy` (Suspense fallback at the router root in `routes.tsx`); `vite.config.ts` `manualChunks` splits leaf vendors (charts/recharts, supabase, radix, motion, icons). Initial JS chunk dropped from 1,708 kB (452 kB gz) to ~162 kB (52 kB gz); recharts (~400 kB) now loads only on the biometrics page. No more >500 kB chunk warning.
  - **Stale-chunk auto-recovery (fix):** after a redeploy, a browser on the old build could fail to load a renamed route chunk ("Failed to fetch dynamically imported module …") and hit the error screen. Now `main.tsx` listens for `vite:preloadError` and `RouteErrorBoundary` detects chunk-load errors — both reload once to fetch the fresh build (10s guard prevents reload loops). Service-worker cache bumped to `mealoptimizer-v2` so stale caches purge on deploy. `main.tsx` also reloads once when a new service worker takes control (`controllerchange`) and re-checks for updates on load/refocus, so deploys propagate instead of getting stuck.
  - **Service worker REMOVED (reliability decision).** After repeated stale-cache "won't load / won't refresh" incidents, the SW was dropped: `main.tsx` no longer registers one and proactively unregisters any existing worker + clears caches; `public/sw.js` is now a self-destructing worker (purges caches, unregisters, reloads clients) so already-installed devices clean themselves up. Trade-off: no offline mode / install prompt, but the app (online-first, all data from Supabase) now always loads the latest deploy — no cache-related failures. The `manifest.webmanifest` + icons remain.
  - **Self-heal watchdog (fix):** an inline script in `index.html` (served fresh, never bundled) checks ~8s after load whether the app mounted; if not (e.g., a device trapped on a broken/old service worker that a hard-refresh can't clear on mobile), it unregisters the SW, clears caches, and reloads once (60s guard). This recovers otherwise-bricked devices automatically. Verified the deployed build itself loads fine (landing + logged-in Home) — the "won't load" cases were stuck-SW devices.
...
4. **Medication–food awareness** — DONE. Curated rules module `src/app/data/medicationInteractions.ts` (warfarin+vitamin-K greens, ACE/ARB & K-sparing + high potassium, BP meds + high sodium, statin+grapefruit, metformin+alcohol, levothyroxine absorption, antibiotic+calcium, iron+tea, MAOI+tyramine) with `getMedicationFoodFlags(medNames, food)`. Wired into the Add-Meal-Log dialog: loads the user's active medications and shows severity-coded warnings both in the food-search panel (nutrient-aware via sodium/potassium) and the manual form (food-name keyword match). Remaining (optional): dedicated interaction-checker screen; expand rules with a pharmacist review.
5. **Local, affordable meal plans** — DONE. `/ai/generate-single-meal` (edge v58) now accepts an optional `budget` (Naira) and prompts Gemini for affordable, in-season, locally-available West African market ingredients, returning `estimatedCostNaira`. `generateSingleMeal(mealType, goal, budget)` in api.ts; PlanMeal.tsx has a ₦ budget input; MealPlanView shows the estimated cost per serving. Remaining (optional): apply the same to the multi-day `/ai/generate-meal-plan`.
6. **Real barcode DB** — DONE. `/barcode/:barcode` (edge v59) now queries OpenFoodFacts first (real product data, per-100g nutrition + sodium) and only falls back to the AI estimate if not found, tagging each result with `source` (openfoodfacts / ai_estimate / not_found). ScanBarcode shows a source badge ("✓ Verified product data" vs "AI estimate — verify the label"). Remaining (optional): let users save corrections into the custom foods table.
7. **Local languages** — IN PROGRESS. `LanguageContext` now supports English, Spanish, French, Yoruba, Igbo, Hausa **and Nigerian Pidgin (new)**; `t()` falls back to English for missing keys. Dictionary expanded with nav + common-action keys; the always-visible **BottomNav is now translated** (switching language in Personalization updates it app-wide). Decision: **curated page-by-page** translation (not runtime machine translation) — safest for a medical app, never touches user data / medical values. **BottomNav + full Login page + full Landing page done** (the whole pre-login experience); dictionary has nav, common-action, auth, and landing keys in all 7 languages. Remaining: wire `t()` through the other pages in batches (next: SignUp, then Home, Health, Profile, then the trackers), expanding the dictionary as we go. Pattern: import `useLanguage`, `const { t } = useLanguage()`, replace hardcoded text with `t('key')`, add the key to `LanguageContext.tsx`. **Streaks (done):** `StreakCard.tsx` computes a daily logging streak (current + longest + last-7-days dots) from meal logs, shown at the top of Home and on Achievements, styled with the amber secondary accent. Remaining adherence idea: medication-dose streaks.
