...
- **Loading skeletons (in progress).** Swapped plain spinners for `SkeletonLoader` components on Biometric Dashboard (`SkeletonDashboard`), Glucose Insights and Grocery List (`SkeletonList`). Goals already used skeletons. Pattern to roll out to remaining data pages (Weight history, Hydration, Sleep, Symptoms, Medication Tracker, Logs, Recipe): import from `../components/SkeletonLoader` and render in the `loading` branch.
