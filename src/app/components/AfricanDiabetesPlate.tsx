...
export default function AfricanDiabetesPlate({
  ...
  return (
    <div className={`rounded-3xl p-5 sm:p-7 bg-white dark:bg-zinc-900 border border-slate-200/90 dark:border-zinc-800 shadow-xl relative overflow-hidden ${className}`}>
      {/* Header with ADA 9-Inch Badge */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="text-[10px] font-black uppercase tracking-wider bg-teal-100 dark:bg-teal-950/60 text-[#126778] dark:text-teal-300 px-2.5 py-0.5 rounded-full border border-teal-200 dark:border-teal-800">
        ...
        <button
          ...
          className="flex items-center gap-1.5 self-start sm:self-auto bg-slate-100 hover:bg-slate-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-slate-700 dark:text-slate-200 px-3 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer shadow-2xs active:scale-95"
      ...
      <div className="flex flex-col md:flex-row items-center justify-center gap-6 sm:gap-8 my-4">
        {/* 9-Inch Plate Circle (280px Diameter Ceramic Silhouette) */}
        <div className="relative w-64 h-64 sm:w-72 sm:h-72 rounded-full p-2 bg-gradient-to-br from-slate-100 via-white to-slate-200 dark:from-zinc-800 dark:via-zinc-850 dark:to-zinc-900 border-4 border-slate-300/80 dark:border-zinc-700 shadow-2xl flex items-center justify-center select-none shrink-0">
          {/* Inner Plate Rim */}
          <div className="w-full h-full rounded-full border-2 border-dashed border-slate-300 dark:border-zinc-700 relative overflow-hidden grid grid-cols-2 grid-rows-2 gap-1 p-1 bg-slate-50 dark:bg-zinc-950/60">
            ...
            <button
              ...
              className="row-span-2 col-span-1 rounded-l-full p-3 bg-gradient-to-br from-emerald-50 to-teal-50/90 dark:from-emerald-950/60 dark:to-teal-950/40 border-r-2 border-emerald-300/80 dark:border-emerald-700 hover:brightness-105 active:scale-98 transition-all flex flex-col items-center justify-center text-center cursor-pointer relative group overflow-hidden shadow-xs"
            >
              <div className="absolute top-2 left-3 bg-emerald-600 text-white text-[9px] font-black px-1.5 py-0.5 rounded-full shadow-2xs">
                50% VEGGIES
              </div>
              <span className="text-3xl sm:text-4xl mb-1 group-hover:scale-110 transition-transform animate-in zoom-in-50 duration-200">
                {selectedVeggie.emoji}
              </span>
              <span className="text-xs font-black text-emerald-950 dark:text-emerald-100 leading-tight px-1">
                {selectedVeggie.name}
              </span>
              <span className="text-[10px] text-emerald-800 dark:text-emerald-300 font-bold mt-0.5">
                🥣 2 Ladles ({selectedVeggie.calories} kcal)
              </span>
              <span className="text-[9px] text-emerald-700/80 dark:text-emerald-400 font-medium">
                Tap to change ⚡
              </span>
            </button>
            ...
            <button
              ...
              className="col-span-1 row-span-1 rounded-tr-full p-2.5 bg-gradient-to-br from-amber-50 to-orange-50/90 dark:from-amber-950/60 dark:to-orange-950/40 border-b-2 border-amber-300/80 dark:border-amber-700 hover:brightness-105 active:scale-98 transition-all flex flex-col items-center justify-center text-center cursor-pointer relative group overflow-hidden shadow-xs"
            >
              <div className="absolute top-1.5 right-3 bg-amber-600 text-white text-[8.5px] font-black px-1.5 py-0.2 rounded-full shadow-2xs">
                25% PROTEIN
              </div>
              <span className="text-2xl sm:text-3xl mb-0.5 group-hover:scale-110 transition-transform">
                {selectedProtein.emoji}
              </span>
              <span className="text-[11px] font-black text-amber-950 dark:text-amber-100 leading-tight truncate max-w-[100px]">
                {selectedProtein.name}
              </span>
              <span className="text-[9.5px] text-amber-800 dark:text-amber-300 font-bold">
            ...
            <button
              ...
              className="col-span-1 row-span-1 rounded-br-full p-2.5 bg-gradient-to-br from-cyan-50 to-sky-50/90 dark:from-cyan-950/60 dark:to-sky-950/40 hover:brightness-105 active:scale-98 transition-all flex flex-col items-center justify-center text-center cursor-pointer relative group overflow-hidden shadow-xs"
            >
              <div className="absolute bottom-1.5 right-3 bg-cyan-700 text-white text-[8.5px] font-black px-1.5 py-0.2 rounded-full shadow-2xs">
                25% SWALLOW
              </div>
              <span className="text-2xl sm:text-3xl mb-0.5 group-hover:scale-110 transition-transform">
                {selectedCarb.emoji}
              </span>
              <span className="text-[11px] font-black text-cyan-950 dark:text-cyan-100 leading-tight truncate max-w-[100px]">
                {selectedCarb.name}
              </span>
              <span className="text-[9.5px] text-cyan-800 dark:text-cyan-300 font-bold">
        ...
        <div className="flex flex-col items-center sm:items-start gap-3.5">
          ...
          <button
            ...
            className="flex items-center gap-3 p-3 bg-gradient-to-r from-sky-50 to-blue-50 dark:from-sky-950/50 dark:to-blue-950/40 rounded-2xl border border-sky-200 dark:border-sky-800 hover:brightness-105 transition-all cursor-pointer shadow-xs active:scale-95 w-full sm:w-60"
          >
            <div className="w-12 h-12 rounded-full bg-sky-500 text-white flex items-center justify-center text-xl shadow-sm shrink-0">
              {selectedDrink.emoji}
            </div>
          ...
          <div className="grid grid-cols-2 gap-2 w-full sm:w-60 text-xs">
            <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-zinc-800/80 border border-slate-200/80 dark:border-zinc-700">
              <span className="text-[10px] font-bold text-slate-500 block">Total Calories</span>
              <span className="text-sm font-black text-slate-900 dark:text-white">
                {totals.calories} <span className="text-[10px] font-normal text-slate-400">kcal</span>
              </span>
            </div>
            <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-zinc-800/80 border border-slate-200/80 dark:border-zinc-700">
              <span className="text-[10px] font-bold text-slate-500 block">Fiber Buffer</span>
              <span className="text-sm font-black text-emerald-600 dark:text-emerald-400">
                {totals.fiber.toFixed(1)}g <span className="text-[10px] font-normal text-slate-400">fiber</span>
              </span>
            </div>
            <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-zinc-800/80 border border-slate-200/80 dark:border-zinc-700">
              <span className="text-[10px] font-bold text-slate-500 block">Lean Protein</span>
              <span className="text-sm font-black text-amber-600 dark:text-amber-400">
                {totals.protein}g
              </span>
            </div>
            <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-zinc-800/80 border border-slate-200/80 dark:border-zinc-700">
              <span className="text-[10px] font-bold text-slate-500 block">K:Na Cardio Ratio</span>
      ...
      <div className={`p-3.5 rounded-2xl border transition-all mt-4 mb-5 ${totals.glycemicStatus.color}`}>
      ...
      <button
        ...
        disabled={isLogging}
        className="w-full py-3.5 px-5 bg-gradient-to-r from-[#126778] via-[#0d9488] to-[#0f766e] hover:brightness-110 active:scale-98 text-white font-black text-sm rounded-2xl shadow-lg transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
      >
      ...
      {activePicker && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-end sm:items-center justify-center p-3 sm:p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-zinc-900 rounded-3xl p-5 sm:p-6 w-full max-w-lg max-h-[85vh] overflow-y-auto border border-slate-200 dark:border-zinc-800 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-zinc-800 pb-3 mb-4">
              ...
              <button
                ...
                className="p-2 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-full text-slate-400 hover:text-slate-600 transition-colors cursor-pointer"
              >
                <X size={18} />
              </button>
            </div>
              ...
              {AFRICAN_PLATE_DATABASE.filter((i) => i.category === activePicker).map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="text-[9.5px] text-slate-600 dark:text-slate-400 mt-2 italic line-clamp-2">
                      {item.clinicalNote}
                    </p>
                  </button>
                );
