...
export default function SmartVideoConcierge({
  ...
  // Auto-prompt once for new users, then stay available as floating bubble
  useEffect(() => {
    try {
      const seen = localStorage.getItem("mealoptimiza_concierge_seen");
      if (!seen) {
        const timer = setTimeout(() => {
          setIsOpen(true);
          localStorage.setItem("mealoptimiza_concierge_seen", "true");
        }, 1500);
  ...
  return (
    <>
      {/* Floating Concierge Bubble (When closed) */}
      <AnimatePresence>
        {!isOpen && (
          <motion.div
          ...
          >
            ...
            >
              <div className="relative w-9 h-9 rounded-full overflow-hidden bg-teal-800 border-2 border-amber-300 shadow-xs flex items-center justify-center text-lg">
                <span>👩🏾‍⚕️</span>
                <span className="absolute bottom-0 right-0 h-2.5 w-2.5 bg-emerald-400 border-2 border-white rounded-full animate-pulse" />
              </div>
              <div className="text-left">
                <span className="text-[10px] font-black text-amber-200 uppercase tracking-wider block leading-none">
                  AI Concierge
                </span>
                <span className="text-xs font-extrabold text-white leading-tight block">
                  Talk with Nurse Amina
                </span>
              </div>
            </button>
          </motion.div>
      ...
      {/* Expanded Interactive Video Concierge Modal */}
      <AnimatePresence>
        {isOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div
              ...
              className="bg-white rounded-3xl overflow-hidden shadow-2xl max-w-md w-full border border-teal-100 flex flex-col max-h-[90vh]"
            >
              {/* Top Header */}
              ...
              <div className="relative bg-slate-950 aspect-video w-full overflow-hidden flex items-center justify-center group">
                {/* Fallback Animated Concierge Graphic */}
                <div className="absolute inset-0 bg-gradient-to-b from-teal-950/80 via-slate-900 to-teal-950 flex flex-col items-center justify-center p-6 text-center">
                  <motion.div
                    ...
                    className="w-20 h-20 rounded-full bg-gradient-to-tr from-[#1f7a8c] to-emerald-400 p-1 mb-2 shadow-lg"
                  >
                ...
                {/* Subtitle / CC Bar */}
                <div className="absolute bottom-10 left-3 right-3 bg-black/75 backdrop-blur-md text-emerald-300 text-[11px] font-medium p-2.5 rounded-xl border border-white/10 shadow-lg text-center leading-snug">
